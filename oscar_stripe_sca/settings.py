...
STRIPE_DEFAULT_TAX_TITLE = getattr(settings, "STRIPE_DEFAULT_TAX_TITLE", "VAT")
STRIPE_ENABLE_TAX_COMPUTATION = getattr(
    settings, "STRIPE_ENABLE_TAX_COMPUTATION", False
)
STRIPE_DEFAULT_PRODUCT_TAX_CODE = getattr(
    settings, "STRIPE_DEFAULT_PRODUCT_TAX_CODE", "txcd_00000000"  # Nontaxable
)
...
STRIPE_OSCAR_WEBHOOK_SECRET = getattr(
    settings, "STRIPE_OSCAR_WEBHOOK_SECRET", None
)
STRIPE_PAYMENT_POLLING_INTERVAL = getattr(
    settings, "STRIPE_PAYMENT_POLLING_INTERVAL", 1000  # in milliseconds
)

STRIPE_ENABLE_RECEIPT_EXPEDITION = getattr(
    settings, "STRIPE_ENABLE_RECEIPT_EXPEDITION", False
)
STRIPE_ENABLE_INVOICE_GENERATION = getattr(
    settings, "STRIPE_ENABLE_INVOICE_GENERATION", False
)
STRIPE_INVOICE_NUMBERING = getattr(
    settings, "STRIPE_INVOICE_NUMBERING", INVOICE_NUMBERING_AUTOMATIC
)
STRIPE_INVOICE_SENDING = getattr(
    settings, "STRIPE_INVOICE_SENDING", INVOICE_SENDING_AUTOMATIC
)
